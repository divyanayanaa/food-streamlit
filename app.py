...
# Utility: Run query and return dataframe
...
# Utility: Execute (INSERT, UPDATE, DELETE)
...
# ---- Streamlit App ----
st.title("Food Sharing Database Dashboard")
...
if choice == "Queries":
    ...
    if query_choice == "Food wastage by type":
        ...
        query = """
            SELECT 
              SUM(CASE WHEN expiry_date < CURDATE() THEN quantity ELSE 0 END) AS expired,
              SUM(CASE WHEN expiry_date >= CURDATE() THEN quantity ELSE 0 END) AS non_expired
            FROM Food_Listings;
        """
        st.dataframe(run_query(query))
    ...
    if crud_table == "Providers":
        if crud_action == "Add":
            with st.form("add_provider"):
                ...
                if submitted:
                    execute_query("INSERT INTO Providers (provider_id, name, provider_type, city, contact) VALUES (%s,%s,%s,%s,%s)",
            ...
            with st.form("update_provider"):
                ...
                if submitted:
                    execute_query("UPDATE Providers SET name=%s WHERE provider_id=%s", (name, pid))
                    st.success("Provider updated successfully")

        elif crud_action == "Delete":
            pid = st.text_input("Provider ID to Delete")
            if st.button("Delete"):
                execute_query("DELETE FROM Providers WHERE provider_id=%s", (pid,))
    ...
    if crud_table == "Receivers":
        if crud_action == "Add":
            with st.form("add_receiver"):
                ...
                if submitted:
                    execute_query("INSERT INTO Receivers (receiver_id, name, type, city, contact) VALUES (%s,%s,%s,%s,%s)",
            ...
            with st.form("update_receiver"):
                ...
                submitted = st.form_submit_button("Update")
                if submitted:
                    execute_query("UPDATE Receivers SET name=%s WHERE receiver_id=%s", (name, rid))
                    st.success("Receiver updated successfully")

        elif crud_action == "Delete":
            rid = st.text_input("Receiver ID to Delete")
            if st.button("Delete"):
                execute_query("DELETE FROM Receivers WHERE receiver_id=%s", (rid,))
    ...
    if crud_table == "Food_Listings":
        if crud_action == "Add":
            with st.form("add_food"):
                ...
                if submitted:
                    execute_query("INSERT INTO Food_Listings (food_id, food_name, quantity, expiry_date, provider_id, food_type, meal_type, location) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)",
            ...
            with st.form("update_food"):
                ...
                submitted = st.form_submit_button("Update")
                if submitted:
                    execute_query("UPDATE Food_Listings SET quantity=%s WHERE food_id=%s", (qty, fid))
                    st.success("Food listing updated successfully")

        elif crud_action == "Delete":
            fid = st.text_input("Food ID to Delete")
            if st.button("Delete"):
                execute_query("DELETE FROM Food_Listings WHERE food_id=%s", (fid,))
    ...
    if crud_table == "Claims":
        if crud_action == "Add":
            with st.form("add_claim"):
                ...
                if submitted:
                    execute_query("INSERT INTO Claims (claim_id, food_id, receiver_id, status, timestamp) VALUES (%s,%s,%s,%s,%s)",
            ...
            with st.form("update_claim"):
                ...
                submitted = st.form_submit_button("Update")
                if submitted:
                    execute_query("UPDATE Claims SET status=%s WHERE claim_id=%s", (status, cid))
                    st.success("Claim updated successfully")

        elif crud_action == "Delete":
            cid = st.text_input("Claim ID to Delete")
            if st.button("Delete"):
                execute_query("DELETE FROM Claims WHERE claim_id=%s", (cid,))
